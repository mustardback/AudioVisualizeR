...
public class TargetSetup : MonoBehaviour {
    ...
    void Start () {
        //Randomize targets behind camera 
        if (Testing == 1)
        {
            ...
            TargetCubeHolder.transform.position = TargetStar.transform.position - new Vector3(-5f, -5f, 0);
        }
        else
        {
        ...
        TargetCubeHolder.transform.forward = TargetCubeHolder.transform.position - MainCamera.transform.position;        
        }
